...
fn output_json(reports: &[VerificationReport]) {
    ...
    println!("{}", serde_json::to_string_pretty(&output).expect("invariant: struct is serializable"));
}
...
fn output_sarif(reports: &[VerificationReport]) {
    ...
    println!("{}", serde_json::to_string_pretty(&sarif).expect("invariant: struct is serializable"));
}
