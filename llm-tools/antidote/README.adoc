...
├── artifacts/           # Additional reset variants (Scheme + Julia)
│   └── llm_reset.jl     # Universal reset artifact (Julia)
├── tools/               # Verification and diagnostic tools (Julia)
│   ├── benchmark.jl     # Benchmark framework for reset effectiveness
│   ├── rsr_compliance.jl # RSR compliance checker
│   ├── llm_diagnostic.jl # Diagnostic tools for context probing
│   └── llm_cli.jl       # CLI interface
├── tests/               # Test suite (Julia)
│   └── test_suite.jl    # Semi-automated test suite
...
*Version*: 1.0.0
*Language*: Julia (rewritten from Python, March 2026)
