...
All notable changes to `patallm-gallery` will be documented in this file.
...
- fix(ci): pin upload-artifact to valid SHA in hypatia-scan.yml (Refs standards#48) (#52)
...
- fix(ci): repair YAML block-scalar in workflow-linter Check Permissions step (#46)
- fix(ci): move secret-scanner Cargo.toml gate from job-level if: to step-level (#47)

### Changed

- refactor(antidote): rewrite llm-antidote tooling from Python to Julia
...
- ci(antipattern): fix top-level dir + benchmark/lsp filename matching (#42)
- ci(antipattern): TS check reads .claude/CLAUDE.md exemption table (#41)
