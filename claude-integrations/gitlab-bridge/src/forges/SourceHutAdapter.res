...
/**
 * SourceHut Forge Adapter
 * Implementation for SourceHut (sr.ht) API
 */

open ForgeAdapter

module SourceHutAdapter: ForgeAdapterType = {
  let forgeName = SourceHut
  let baseUrl = "https://git.sr.ht/api"

  // SourceHut has a different workflow - uses mailing lists and patches
  // This adapter provides a bridge to work with their API
  ...
  let getIssue = async (~projectId: string, ~issueId: int): result<issue, string> => {
    // SourceHut uses todo.sr.ht for issue tracking
    Error("Not implemented")
  }
  ...
  let getMR = async (~projectId: string, ~mrId: int): result<mergeRequest, string> => {
    // SourceHut uses email-based patches instead of web PRs
    Error("Not implemented")
  }
