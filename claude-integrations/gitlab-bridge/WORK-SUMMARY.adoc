...
**Scope:** Complete codebase conversion from  to 
...
|`src/auth/Errors.res`
|Error types using  exception system
...
|Anthropic SDK bindings for 

|`src/bindings/Express.res`
|Express.js bindings for 
...
4. *Fast Compilation:*  compiler is much faster than 
5. * Runtime:* Modern runtime with built-in  support

**Language Policy Compliance:**

- ✅ No  (converted to )
- ✅ No Node.js (using )
- ✅ No npm (using  imports)
- ✅  as primary language
...
- Updated bash examples to use ` task` instead of `npm`
- Changed code examples to  syntax
- Updated project structure to show  files
...
- Copyright: 2025 Jonathan D.A. Jewell <j.d.a.jewell@open.ac.uk>
...
*  provides superior type safety over 
*  eliminates need for node_modules and npm
* Palimpsest License (MPL-2.0) applied throughout
